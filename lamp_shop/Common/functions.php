<?php
	 ...
	 * @param [type] number $zoomHeight [description] 如果不为空，就是制定宽高缩放
	 * @return null
	 */

	function zoom($imagePath,$dstDir,$zoomSize,$zoomHeight = null)
	{
        //处理目标目录
        $dstDir = rtrim($dstDir,'/') . '/';
		...
		*/
		...
		// 随机图片名
		$imageName = md5( mt_rand(1,9999999) . uniqid()) .'.' .$hz;
        // 另存为图片
		$imageecho($canvas, $dstDir . $imageName);
	...
	// 专门用于数据库的查询
	function query($sql,$multiMode = false){
		// 1.连接
		$link = @mysqli_connect(HOST,USER,PASS,DB) or exit('连接失败！');
		// 设置字符集
		mysqli_set_charset($link,CHARSET);
		...
		// 发送执行
        $result = mysqli_query($link,$sql);
        //echo '错误号：'.mysqli_errno($link) . '<br>' . mysqli_error($link);
        ...
        if($result)
        {
            ...
            $multiResult = [];
            $multiResult['list'] = $list;
            $multiResult['total'] = mysqli_num_rows($result);
            $multiResult['errno'] = mysqli_errno($link);
            $multiResult['error'] = mysqli_error($link);
